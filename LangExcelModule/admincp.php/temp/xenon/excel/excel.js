...
excel.view = function(data) {
	if(typeof(loadExcelModel)!=="undefined") {
		document.body.querySelector(".executeExcel").className = document.body.querySelector(".executeExcel").className.replace(" hide", "");
		document.body.querySelector(".selectAllExcel").className = document.body.querySelector(".selectAllExcel").className.replace(" hide", "");
	}
	...
	if(typeof(loadExcelModel)!=="undefined") {
		...
		el.className = "excel-line header";
		el.excel = JSON.parse(JSON.stringify(column));
		el.innerHTML += '<span></span>';
		for(var j=0; j<data[0].length;j++) {
			 el.innerHTML += '<span>'+data[0][j]+'</span>';
			 el.excel[column_keys[j]] = data[0][j];
		}
...
excel.send = function(k) {
	...
	var url;
	if(typeof(loadExcelModel)!=="undefined") {
		url = default_localadmin_link+loadExcelModel
	} else {
		url = default_localadmin_link+"?pages=LangExcel&save="+excel.langLoad
	}
	ajax({ "url": url, "data": data }, function(d) {
		d = JSON.parse(d);
		if(d["error"] == false) {
			...
			}else if(typeof(notifiedSuccess)!=="undefined") {
				ajax({"url": notifiedSuccess}, function(d) {});
			}
